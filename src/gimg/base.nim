...
proc extractData(data: JsonNode): ImagesResult =
  ...
  for data in data{31, 0, 12, 2}:
    let img = data{1}
    var image: ImageResult

    block desc:
      let descData = img{9}{"2003"}
      image.title = descData{3}.getStr
      image.site = descData{2}.getStr
      try: # credits
        let creditData = descData{18}
        image.credit = creditData{0}.getStr
        let copyrightData = creditData{1}
        image.author = copyrightData{0}.getStr
        image.copyright = creditData{3}.getStr
      except: discard
      image.description = img{9}{"2006"}{1}{4}.getStr
    block thumb:
      let thumbData = img{2}
